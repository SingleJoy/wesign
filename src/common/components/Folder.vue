    ...
    <div class="folder-content">
      ...
      <div class="folder-list">
        ...
        <div class="list-item" style="width: 980px;">
          <div v-for="(itemList,indexList) in folderList" :key="indexList">
            <div class="for-folder-list"
                 :style="{'transform':'translateX('+(indexList+nowIndex)*980+'px)','transition-duration': '1.5s'}" v-if="folderListShow">
              <ul >
                <li v-for="(item ,index) in itemList" :key="index" :class="{'active':(item.filingNo==$store.state.showFilingNo)}">
      ...
      contractFilings(){
        contractFilings(this.interfaceCode,this.accountCode).then(res=>{
          if(res.data.resultCode=='1'){
            ...
            let array=[];
            for(let i=0;i<this.folderList.length;i+=10){
              array.push(this.folderList.splice(0,10))
            }
