    ...
    <div class="EnterprisePayments">
      <div class="step" style="width:420px;">
        <ul>
          ...
          <li class="active"><i class='el-icon-edit'></i><b>实名认证</b></li>
          <p></p>
        ...
        <div class="payment-content" >
          ...
          <el-form :model="ruleForm" :rules="rules" ref="ruleForm" class="account-ruleForm" size="medium">
            ...
            <el-form-item label="开户行支行所在省市" :label-width="formLabelWidth" prop="bankArea">
              <el-cascader
                @change="change" :options="options" v-model="ruleForm.bankArea" style="width: 330px;height:40px;"></el-cascader>
            </el-form-item>
          ...
          <div class="edit-btn" >
            ...
            <el-button style="width: 200px;height: 40px;background:#ccc;" >返回</el-button>
  ...
  export default {
    ...
    data () {
      ...
      var validateBankAccount=(rule,value,callback)=>{
        ...
        }else if(TrimAll(value)!==''&&validateBankAccountNumber(value)){
          callback(new Error('企业银行账号格式错误，仅可录入数字'));
        }else {
          callback()
        }
      }
      ...
      var validateBankArea=(rule,value,callback)=>{
        if (value === '') {
          callback(new Error('请输入开户行支行所在省市'));
        }else{
          callback()
        }
      }

      var validateBankBranchName=(rule,value,callback)=>{
        if (value === '') {
          callback(new Error('请输入支行银行名称'));
        }else{
          callback()
        }
      }
      ...
      return {
        ...
        rules:{
          ...
          bankArea:[
            { required: true,validator: validateBankArea, trigger: 'blur'}
          ],
          bankBranchName:[
            { required: true, validator: validateBankBranchName, trigger: 'blur' }
          ],
        },
