/*
 * @Author: wangjia
 * @Date: 2018-06-04 11:06:39
 * @Last Modified by: wangjia
 * @Last Modified time: 2018-07-03 18:00:29
 */
<template>
  <div>
  <div class="Tops">
...
export default {
	...
	computed: {
		...
		pages: function() {
			var pag = [];
			this.showItem = 10;
			if (this.currentIndex < this.showItem) {
				//如果当前的激活的项 小于要显示的条数
				//总页数和要显示的条数那个大就显示多少条
	...
	methods: {
		...
		this.$store.dispatch("fileSuccess1", {
			...
			contractNo: this.$store.state.contractNo1
		});
		this.$store.dispatch("type", { type: "back" });
		sessionStorage.setItem(
			"contractName",
			JSON.stringify(this.$store.state.contractName1)
		);
		sessionStorage.setItem(
			"contractNo",
			JSON.stringify(this.$store.state.contractNo1)
		);
		...
		var len = document.getElementById("more").getElementsByTagName("dl")
			.length; //获取签署人个数
		var param = "";
		var arr = [];
		...
		for (var i = 2; i < sign_length.length; i++) {
			var elementNum = sign_length[i].childNodes[8].innerText
			.replace(/[^0-9\-,]/g, "")
			.split("")
			.join("");
			arr.push(elementNum);
		...
		for (var i = 2; i < sign_list.length; i++) {
			var elementNum = sign_list[i].childNodes[8].innerText
			.replace(/[^0-9\-,]/g, "")
			.split("")
			.join("");
			arr.push(elementNum);
	...
	directives: {
		...
		bind: function(el, binding) {
			...
			let disY =
				e.clientY - el.offsetTop + document.documentElement.scrollTop;
			...
			document.onmousemove = function(e) {
				...
				document.getElementById("div1").offsetLeft -
				el.clientWidth; //最大右部距离
				...
				"<br>" +
				'</h5><h6 class="textStyle">' +
				el.childNodes[2].innerText +
				'</h6><input type="hidden" class="user" value="' +
				el.childNodes[4].innerText +
				'"><input type="hidden" class="user" value="' +
				el.childNodes[6].innerText +
				'"><b  class="delete">X</b>';
				...
				document.onmouseup = function(e) {
					...
					.getElementById("div2")
					.style.transform.match(/\.*translate\((.*?)\)/)[1]
					.replace(/[^0-9\-,]/g, "")
					.split(",")[1];
